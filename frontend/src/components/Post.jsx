...
const Post = ({ post }) => {
  ...
  const likeOrDislikeHandler = async () => {
    try {
      ...
      const res = await axios.get(
        `${import.meta.env.VITE_APP_API_KEY}/api/v1/post/${
          post?._id
        }/${action}`,
        {
  ...
  const commentHandler = async () => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_APP_API_KEY}/api/v1/post/${post?._id}/comment`,
        { text },
        {
          // headers: {
  ...
  const deletePostHandler = async () => {
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_APP_API_KEY}/api/v1/post/delete/${post?._id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
          withCredentials: true,
        }
  ...
  const bookmarkHandler = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_APP_API_KEY}/api/v1/post/${post?._id}/bookmark`,
